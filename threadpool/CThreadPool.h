...
private:
    ...
    class CThreadMutex
    {
        ...
        public:
            CThreadMutex(){ pthread_mutex_init(&this_lock, NULL); }
            ~CThreadMutex(){ pthread_mutex_destroy(&this_lock); }
            void Lock(){ pthread_mutex_lock(&this_lock); }
            void Unlock(){ pthread_mutex_unlock(&this_lock); }
    };
...
private: 
    ...
    unsigned int m_ToNum;  //zmc assert all tasks are done! 
...
//    vector<CWorkerThread*>   m_ThreadList; 
    ...
    int     GetAllNum(void){return m_BusyList.size() + m_AvailNum;}
